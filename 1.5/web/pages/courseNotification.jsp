...
<title><fmt:message key="courseDetail.title" />
</title>
<content tag="heading">
...
<form method="post" action="<c:url value="/notifyCourse.html"/>"
	id="courseForm" onsubmit="return validateCourse(this)">
	...
	<c:if test="${enableMail}">
		...
		<c:choose>
			<c:when test="${!waitinglist}">
				<div class="message" style="font-size: 12px">
					<fmt:message key="courseNotification.askmailnotification" />
				</div>
			</c:when>
			<c:otherwise>
				<div class="message" style="font-size: 12px">
					<fmt:message key="courseNotification.askwaitinglistmail" />
				</div>
			</c:otherwise>
		...
		<h4>
			<fmt:message key="courseNotification.mailheading" />
		</h4>
		<table class="detail">
			<tr>
				<th>
			...
			<tr>
				<td colspan="2" class="buttonBar">
					...
					<c:if
						test="${isAdmin || isEducationResponsible || (isCourseResponsible && username == course.responsibleUsername)}">
						<button type="button"
					...
					<input type="submit" class="button" name="skip"
					...
					<c:if test="${!enableMail}">
						...
						<h4>
							<fmt:message key="courseNotification.confirmheading" />
						</h4>
						<table class="detail">
							<tr>
								<td colspan="2" class="buttonBar">
									...
									<input type="submit" class="button" name="confirm"
										onclick="bCancel=true"
						...
						<h4>
							<fmt:message key="courseNotification.coursedetails" />
						</h4>
