...
public class CourseController extends BaseFormController {
    ...
    protected Map referenceData(HttpServletRequest request, Object command, Errors errors) throws Exception {
		...
		if (!StringUtils.isBlank(stopTimeString)) {
			try {
				...
				// legger p� 24timer for � sikre til-og-med sluttdato
        ...
        if(ApplicationResourcesUtil.isSVV()){ // alle LDAP-brukere ser alle m�ter (kurs), men endringstilgang styres av JSP.
        	Integer[] status = null; 
    ...
    /**
     * Metode for � sette parametre i modell som benyttes av jsp for � vise/skjule kolonner
     * @param status
     * @param courseList
     * @param model
     */
    ...
    public ModelAndView onSubmit(HttpServletRequest request, HttpServletResponse response, Object command, BindException errors)
		...
		if(!StringUtils.isBlank(postalcode)){
			if(postalcode.length() != 4 || lessThanZeroOrNAN(postalcode)){
				...
				// h�ndterer kun grenseverdier slik at s�k mot ikke-eksisterende x000 kun gir s�k oppover
				// tilsvarende for ikke eksisterene x999 som kun vil gi s�k nedover
				boolean doNotGoUp = pc % 1000 == 999;
				boolean doNotGoDown = pc % 1000 == 0;
				
				while(approx){
					// oppgitt postnummer finnes ikke - vi s�ker i omkringliggende nr-serier
					numberOfTries++;
		...
		if (ApplicationResourcesUtil.isSVV()) { // alle LDAP-brukere ser alle m�ter (kurs), men endringstilgang styres av JSP.
			...
			if (StringUtils.isBlank(postalcode)) {
				...
				model.put("containsUnfinished", new Boolean(true));
				model.put("containsFinished", new Boolean(false));
		        model.put("enableExport", new Boolean(false)); // deaktivering av fileksport for postnummers�keresultat
				
				// melding til bruker som forklarer postnr-s�k
