...
//			log.info("Synkronisering av " + antall + " brukere ferdig!");
			...
			/* Alternativ l�sning for synkronisering av ezBrukere (med basis i "java"-database fremfor "eZ"-database
			 * Brukere blir uansett lagt til i APP_USER-tabellen ved f�rste innlogging, det b�r derfor v�re ok � 
...
//    		// mulig dette egentlig er un�dvendig, men oppstod pga. inkonsistens mellom frikomdb og ezdb i utviklingsmilj�   		
    		try {
				...
				if(!current.getUsername().equalsIgnoreCase(emailuser.getUsername())) {
					// Gj�r nokke med gammel bruker
					byttNavnOgDisable(emailuser, false);
				}
			} catch (Exception e) {
				if(emailuser != null) {
					// Det finnes allerede ein bruker med nonexist.no adresse, m� slettes.
