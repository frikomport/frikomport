...
public class LabelTag extends TagSupport {
    ...
    public int doStartTag() throws JspException {
        ...
        if ((message == null) || "".equals(message.trim())) {
            ...
            if ((fes != null) && (fes.size() > 0)) {
                if (helpTip) {
                    label.append(
                        " <a class=\"errorLink\" href=\"?\" onclick=\"showHelpTip(event, '");
                    label.append(errorMsg + "', false); return false\" ");
                    label.append("onmouseover=\"showHelpTip(event, '");
                    label.append(errorMsg + "', false); return false\" ");
                    label.append(
                        "onmouseout=\"hideHelpTip(event); return false\">");
                }
    ...
    private ValidatorResources getValidatorResources() {
        ...
        try {
            factory = (ValidatorFactory) BeanFactoryUtils.beanOfTypeIncludingAncestors(ctx,
